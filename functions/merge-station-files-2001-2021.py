...
listCC = glob.glob('/.../ecad/ECA_blend_cc/CC_*.txt')
listQQ = glob.glob('/.../ecad/ECA_blend_qq/QQ_*.txt')
listTN = glob.glob('.../ecad/ECA_blend_tn/TN_*.txt')
listTX = glob.glob('/.../ecad/ECA_blend_tx/TX_*.txt')
...
ceres= xr.open_dataset('/.../ceres/CERES_SYN1deg-Day_Terra-Aqua-MODIS_Ed4.1_Subset_20000301-20221231.nc')#.sel(time = slice(time_slice[0], time_slice[1]))
ceresaod = ceres.ini_aod55_daily
ceresclt = ceres.cldarea_total_daily

EOBStmin = xr.open_dataset('/.../eobs/tn_ens_mean_0.25deg_reg_v27.0e.nc').rename({'longitude':'lon','latitude':'lat'})
EOBStmax = xr.open_dataset('/.../eobs/tx_ens_mean_0.25deg_reg_v27.0e.nc').rename({'longitude':'lon','latitude':'lat'})
EOBSsw = xr.open_dataset('/.../eobs/qq_ens_mean_0.25deg_reg_v27.0e.nc').rename({'longitude':'lon','latitude':'lat'})
...
for k in idsmerged:
    ...
    fileCC = glob.glob('/.../ecad/ECA_blend_cc/CC_*0'+str(k)+'.txt')[0]
    fileQQ = glob.glob('/.../ecad/ECA_blend_qq/QQ_*0'+str(k)+'.txt')[0]
    fileTN = glob.glob('/.../ecad/ECA_blend_tn/TN_*0'+str(k)+'.txt')[0]
    fileTX = glob.glob('/.../ecad/ECA_blend_tx/TX_*0'+str(k)+'.txt')[0]
    ...
    if (np.max(_cc) > 20010101) & (np.max(_qq) > 20010101) & (np.max(_tn) > 20010101) & (np.max(_tx) > 20010101):
        
        _max = np.min([np.max(_cc), np.max(_qq), np.max(_tn), np.max(_tx)])
    
        if _max > 20211231: _max = 20211231
    
        ccs = np.where(_cc >= 20010101)[0][0]
        qqs = np.where(_qq >= 20010101)[0][0]
        tns = np.where(_tn >= 20010101)[0][0]
        txs = np.where(_tx >= 20010101)[0][0]
        ...
        aod = get_nearest_grid(ceresaod, stations.loc[np.where(stations.id == int(k))]['lat'].values[0], stations.loc[np.where(stations.id == int(k))]['lon'].values[0])
        df['ceresAOD'] = aod.sel(time = slice(pd.to_datetime(str(_min), format = '%Y%m%d'), pd.to_datetime(str(_max+0.8), format = '%Y%m%d')+datetime.timedelta(days =1))).values
        
        clt = get_nearest_grid(ceresclt, stations.loc[np.where(stations.id == int(k))]['lat'].values[0], stations.loc[np.where(stations.id == int(k))]['lon'].values[0])
        df['ceresCLT'] = clt.sel(time = slice(pd.to_datetime(str(_min), format = '%Y%m%d'), pd.to_datetime(str(_max+0.8), format = '%Y%m%d')+datetime.timedelta(days =1))).values
        
        df = df.replace(-9999, np.nan)
        df[['CC','QQ','TN','TX','ceresAOD','ceresCLT']] = df[['CC','QQ','TN','TX','ceresAOD','ceresCLT']].mask(df.CC.isna() | df.QQ.isna() | df.TN.isna()| df.TX.isna(), np.nan)
        
        if all(df.count() > 3600):
               df.to_xarray().to_netcdf('/data/merged-stations-20010101-20211231/'+str(k)+'.nc')