...
def causal_grid_box(tn,tx,qq,clt,aod, lat, lon):
    ...
    if np.where(~np.isnan(yr_nan))[0].shape[0] >= 10:
        ...
        # print(results)
        plt.savefig('/home/s2135337/carla/results/eobs/v26/graphs/'+str(lat[0])+'-'+str(lon[0])+'.png')
        np.save('/home/s2135337/carla/results/eobs/v26/results/'+str(lat[0])+'-'+str(lon[0]),np.array(list(results.items()))[:,1])
...
xr.apply_ufunc(causal_grid_box, ee.tn26, ee.tx26, ee.qq26, ee.ceresCLT, ee.ceresAOD, ee.lattt, ee.lonnn, input_core_dims=[["time"],["time"],["time"],["time"],["time"],["time"],["time"]], dask = 'allowed', vectorize = True)