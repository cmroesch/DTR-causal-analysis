...
aod = xr.open_dataset('/home/s2135337/carla/cmip6/od550aer_day_HadGEM3-GC31-LL_r1_18500101-20141230.nc').sel(time = slice('1950-01-01','2014-12-30')).where(mask).od550aer
...
for j in ccc.lon.values[np.concatenate([np.where(ccc.lon.values >= 330)[0],np.where(ccc.lon.values <= 30)[0]])]:
    for i in ccc.lat.values:
        ...
        if not any(np.isnan(cs)):
            ...
            # ce_estimation_wright(data, '/home/s2135337/carla/results/cmip6/hadgem3/stations/','results/'+str(i)+'-'+str(j)+'.npy','ci/'+str(i)+'-'+str(j)+'.npy','graphs/'+str(i)+'-'+str(j)+'.png', graph)
...
# plt.savefig('//home/s2135337/carla/results/cmip6/hadgem3/stations/europe-stations-nolag.png')
# np.save('/home/s2135337/carla/results/cmip6/hadgem3/stations/europe-stations-nolag.npy',np.array(list(resultss.items()))[:,1])
