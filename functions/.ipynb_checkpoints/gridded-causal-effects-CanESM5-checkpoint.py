...
def causal_grid_box(tn,tx,qq,clt,aod,lat,lon):
    ...
    yr_nan = qq.reshape(14,365)[:,j1:j2].mean(axis =1)*tn.reshape(14,365)[:,j1:j2].mean(axis =1)*tx.reshape(14,365)[:,j1:j2].mean(axis =1)*clt.reshape(14,365)[:,j1:j2].mean(axis =1)*aod.reshape(14,365)[:,j1:j2].mean(axis =1)
    print(np.where(~np.isnan(yr_nan))[0].shape[0])
    
    if np.where(~np.isnan(yr_nan))[0].shape[0] >= 10:
        var_names = ['tmin','tmax','sw','cloudiness','rh','aod']
        dataStacked = np.stack([tn.reshape(14,365)[np.where(~np.isnan(yr_nan))[0],j1:j2],
                                tx.reshape(14,365)[np.where(~np.isnan(yr_nan))[0],j1:j2],
                                qq.reshape(14,365)[np.where(~np.isnan(yr_nan))[0],j1:j2],
                                clt.reshape(14,365)[np.where(~np.isnan(yr_nan))[0],j1:j2],
                                aod.reshape(14,365)[np.where(~np.isnan(yr_nan))[0],j1:j2]])
        
        data = {i: dataStacked[:,i,:].transpose() for i in range(np.where(~np.isnan(yr_nan))[0].shape[0])}

        results = ce_estimation(data)

        # np.save('/home/s2135337/carla/results/cmip6/causal-effects-CanESM5-01-14/'+str(round(lat[0],3))+'-'+str(round(lon[0],3)),results)
        np.save('/home/s2135337/carla/results/cmip6/causal-effects-CanESM5-01-14/'+str(lat[0])+'-'+str(lon[0]),results)
...
ee = xr.open_dataset('/home/s2135337/carla/cmip6/CanESM5_all.nc')

# ee = e.sel(time=~((e.time.dt.month == 2) & (e.time.dt.day == 29)))


dime = np.zeros((5110, 2, 18,42))*np.nan
for i in range(18):
    for j in range(42):
