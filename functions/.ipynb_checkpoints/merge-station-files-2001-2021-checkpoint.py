...
ceres= xr.open_dataset('/exports/geos.ed.ac.uk/climate_change/users/carla/ceres/CERES_SYN1deg-Day_Terra-Aqua-MODIS_Ed4.1_Subset_20000301-20221231.nc')#.sel(time = slice(time_slice[0], time_slice[1]))
...
for k in idsmerged:
    ...
    if (np.max(_cc) > 20010101) & (np.max(_qq) > 20010101) & (np.max(_tn) > 20010101) & (np.max(_tx) > 20010101):
        ...
        df[['CC','QQ','TN','TX','ceresAOD','ceresCLT','claraCLT','merraAOD']] = df[['CC','QQ','TN','TX','ceresAOD','ceresCLT','claraCLT','merraAOD']].mask(df.CC.isna() | df.QQ.isna() | df.TN.isna()| df.TX.isna(), np.nan)
        
        if all(df.count() > 3600):
               df.to_xarray().to_netcdf('/exports/geos.ed.ac.uk/climate_change/users/carla/ecad/merged-stations-20010101-20211231/'+str(k)+'.nc')