...
def causal_grid_box(tn,tx,qq,clt,aod,lat,lon):
    ...
    if np.where(~np.isnan(yr_nan))[0].shape[0] >= 10:
        ...
        np.save('/home/s2135337/carla/results/eobs/causal-effects-v27-01-14/'+str(lat[0])+'-'+str(lon[0]),results)
...
e = xr.open_dataset('/home/s2135337/carla/ceres-clara-merra-eobs27-eobs26-merged-20010101-20211231.nc').sel(time = slice('2001-01-01','2014-12-31'))

ee = e.sel(time=~((e.time.dt.month == 2) & (e.time.dt.day == 29)))


dime = np.zeros((5110, 2, 140,280))*np.nan
for i in range(140):
    for j in range(280):
